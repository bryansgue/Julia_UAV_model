...
function Angulo(ErrAng)
    # Función "Angulo" que restringe el ángulo entre [0 : pi] y [-pi : 0]
...
function main()
    ...
    J = zeros(4,4)
    global K1 = 0.5*Matrix{Float64}(I, 4, 4)
    global K2 = 0.5*Matrix{Float64}(I, 4, 4)
    global K3 = 0.5*Matrix{Float64}(I, 4, 4)
    global K4 = 0.5*Matrix{Float64}(I, 4, 4)

    t = 0:ts:samples
    mul = 5
    xd(t) = 5 * sin(mul*0.04*t)+0.1
    yd(t) = 5 * sin(mul*0.08*t)+0.1
    zd(t) = 1 * sin(0.08*t) +5  

    xdp(t) = mul*5*0.04*cos(mul*0.04*t);
    ydp(t) = mul*5*0.08*cos(mul*0.08*t); 
    zdp(t) = 0.08*cos(0.08*t);

    xdpp(t) = -5*0.04*0.04*mul*mul*sin(mul*0.04*t);
    ydpp(t) = -5*0.08*0.08*mul*mul*sin(mul*0.08*t); 
